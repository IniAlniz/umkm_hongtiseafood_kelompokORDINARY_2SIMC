<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Add Product Image') }} : {{$product->name}}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.products.product-images.store', $product) }}">
                        @csrf

                        <!-- Image Url -->
                        <div>
                            <x-input-label for="image_url" :value="__('Image URL')" />
                            <x-text-input id="image_url" class="block mt-1 w-full" type="text" name="image_url" :value="old('image_url')" required autofocus autocomplete="image_url" />
